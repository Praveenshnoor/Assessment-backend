...
io.on('connection', (socket) => {
    ...
    socket.on('admin:send-message', async (data) => {
        ...
        if (!studentId || !message) {
            ...
            socket.emit('admin:message-failed', { 
                studentId, 
        ...
        if (!studentSession) {
            ...
            socket.emit('admin:message-failed', { 
                studentId: studentIdStr, 
                error: 'Student not in active session' 
            });
            return;
        }
    ...
    socket.on('admin:get-message-history', async (data) => {
        ...
        try {
            const result = await pool.query(
                `SELECT id, admin_id, message, message_type, priority, 
                        message_timestamp, read_status, read_at
